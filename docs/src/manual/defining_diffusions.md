...
```julia
P_f64 = Lorenz(10.0, 8.0/3.0, 28.0, 0.2)
P_f32 = Lorenz(10.0f0, 8.0f0/3.0f0, 28.0f0, 0.2)
```
...
```
!!! note
    The split into `constant` and `variable` parameters is not done at a compile time and can be also done by hand after the `struct` with the diffusion has already been defined (though, is discouraged). For instance, in the `Lorenz` example above we have:

    ```julia
    julia> DD.parameters(P)
    (:p1 => 10.0, :p2 => 2.6666666666666665, :p3 => 28.0, :σ => 0.2)

    julia> DD.const_parameters(P)
    ()

    julia> DD.var_parameters(P)
    (:p1 => 10.0, :p2 => 2.6666666666666665, :p3 => 28.0, :σ => 0.2)
    ```

    If we wanted to change our mind and define, say, `:p1` and `:p3` as `constant` parameters we could do that by overwriting the definitions of the three functions above, i.e.

    ```julia
    DD.parameters(P::Lorenz) = (:p1 => P.p1, :p2 => P.p2, :p3 => P.p3, :σ => P.σ) # this actually doesn't change anything in our example
    DD.const_parameters(P::Lorenz) = (:p1 => P.p1, :p3 => P.p3)
    DD.var_parameters(P::Lorenz) = (:p2 => P.p2, :σ => P.σ)
    ```
